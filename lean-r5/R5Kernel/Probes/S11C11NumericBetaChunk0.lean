...
theorem kernel_c11_canonical_beta_chunk0 : ∀ i : Fin 32, ∀ j : Fin 198,
    kernelC11CycleTypeAt j ⊆ r5RepMaskSet11C11
        (kernelC11Row (kernelC11CanonicalRep126
          ⟨i.val, Nat.lt_of_lt_of_le i.isLt (by decide)⟩).val).mask →
      (kernelC11CycleTypeAt j).card ≤ (kernelC11Row
        (kernelC11CanonicalRep126
          ⟨i.val, Nat.lt_of_lt_of_le i.isLt (by decide)⟩).val).beta := by
  decide +kernel

run_cmd R5Kernel.checkStandardAxioms ``kernel_c11_canonical_beta_chunk0
