...
theorem kernel_c11_canonical_beta_chunk2 : ∀ i : Fin 32, ∀ j : Fin 198,
    kernelC11CycleTypeAt j ⊆ r5RepMaskSet11C11
        (kernelC11Row (kernelC11CanonicalRep126
          ⟨i.val + 64, by omega⟩).val).mask →
      (kernelC11CycleTypeAt j).card ≤ (kernelC11Row
        (kernelC11CanonicalRep126
          ⟨i.val + 64, by omega⟩).val).beta := by
  decide +kernel

run_cmd R5Kernel.checkStandardAxioms ``kernel_c11_canonical_beta_chunk2
