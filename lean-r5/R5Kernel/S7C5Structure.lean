...
def kernelC5Embedding : Fin 5 ↪ Fin 7 where
  toFun x := ⟨x.val, by omega⟩
  inj' := by intro x y h; exact Fin.ext (congrArg (fun z : Fin 7 => z.val) h)

def kernelC5Slice (I : Finset (Fin 7)) : Finset (Fin 5) :=
  Finset.univ.filter (fun x => kernelC5Embedding x ∈ I)

theorem kernel_c5_slice_map (I : Finset (Fin 7)) :
...
theorem kernel_c5_slice_independent (I : Finset (Fin 7))
    (hI : (cycleGraph 7 5).IsIndepSet (I : Set (Fin 7))) :
  ...
  change _ ≠ _ ∧ (cycleRel 7 5 _ _ ∨ cycleRel 7 5 _ _)
  change x ≠ y ∧ (cycleRel 5 5 x y ∨ cycleRel 5 5 y x) at hadj
  exact ⟨fun h => hxy (kernelC5Embedding.injective h), hadj.2⟩

def kernelIsoEmbedding : Fin 2 ↪ Fin 7 where
  toFun x := ⟨x.val + 5, by omega⟩
  inj' := by
    intro x y h
    have hv := congrArg (fun z : Fin 7 => z.val) h
    apply Fin.ext
    change x.val + 5 = y.val + 5 at hv
    omega

def kernelIsoSlice (I : Finset (Fin 7)) : Finset (Fin 2) :=
  Finset.univ.filter (fun x => kernelIsoEmbedding x ∈ I)

def kernelJoinParts (p : Finset (Fin 5) × Finset (Fin 2)) : Finset (Fin 7) :=
  p.1.map kernelC5Embedding ∪ p.2.map kernelIsoEmbedding

def kernelC5Product : Finset (Finset (Fin 5) × Finset (Fin 2)) :=
  kernelC5Independents ×ˢ Finset.univ

theorem kernel_iso_slice_map (I : Finset (Fin 7)) :
...
theorem kernel_join_slices (I : Finset (Fin 7)) :
...
theorem kernel_s7_types_covered :
    cycleTypes 7 5 ⊆ kernelC5Product.image kernelJoinParts := by
  intro I hI
...
theorem kernel_s7_sum_le_cover (F : Finset (Fin 7) → ℕ) :
    (∑ I ∈ cycleTypes 7 5, F I) ≤ ∑ p ∈ kernelC5Product, F (kernelJoinParts p) := by
  exact (Finset.sum_le_sum_of_subset kernel_s7_types_covered).trans
    (Finset.sum_image_le_of_nonneg (fun _ _ => Nat.zero_le _))


theorem kernel_s7_beta_of_cycle_bound (R : Finset (Fin 7)) (a b : ℕ)
    (hcycle : ∀ J : Finset (Fin 5),
      (cycleGraph 5 5).IsIndepSet (J : Set (Fin 5)) →
      J ⊆ kernelC5Slice R → J.card ≤ a)
    (hab : a + (R.filter (fun x => ¬ x.val < 5)).card ≤ b)
    (I : Finset (Fin 7)) (hI : (cycleGraph 7 5).IsIndepSet (I : Set (Fin 7)))
    (hIR : I ⊆ R) : I.card ≤ b := by
...
run_cmd R5Kernel.checkStandardAxioms ``kernel_s7_sum_le_cover
run_cmd R5Kernel.checkStandardAxioms ``kernel_s7_beta_of_cycle_bound
