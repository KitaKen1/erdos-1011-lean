...
theorem kernel_c7_slice_independent (I : Finset (Fin 7))
    (hI : (cycleGraph 7 7).IsIndepSet (I : Set (Fin 7))) :
...
def kernelIsoSlice (I : Finset (Fin 7)) : Finset (Fin 0) :=
  Finset.univ.filter (fun x => kernelIsoEmbedding x ∈ I)

def kernelJoinParts (p : Finset (Fin 7) × Finset (Fin 0)) : Finset (Fin 7) :=
  p.1.map kernelC7Embedding ∪ p.2.map kernelIsoEmbedding

def kernelC7Product : Finset (Finset (Fin 7) × Finset (Fin 0)) :=
...
theorem kernel_s7_types_covered :
    cycleTypes 7 7 ⊆ kernelC7Product.image kernelJoinParts := by
  intro I hI
...
theorem kernel_s7_sum_le_cover (F : Finset (Fin 7) → ℕ) :
    (∑ I ∈ cycleTypes 7 7, F I) ≤ ∑ p ∈ kernelC7Product, F (kernelJoinParts p) := by
...
theorem kernel_s7_beta_of_cycle_bound (R : Finset (Fin 7)) (a b : ℕ)
    ...
    (I : Finset (Fin 7)) (hI : (cycleGraph 7 7).IsIndepSet (I : Set (Fin 7)))
    (hIR : I ⊆ R) : I.card ≤ b := by
