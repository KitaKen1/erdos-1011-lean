...
def kernelS7Split : Fin 7 ⊕ Fin 0 ≃ Fin 7 := finSumFinEquiv

def kernelBlockEquiv (a : Fin 7 ≃ Fin 7) (b : Fin 0 ≃ Fin 0) : Fin 7 ≃ Fin 7 :=
  kernelS7Split.symm.trans ((Equiv.sumCongr a b).trans kernelS7Split)

theorem kernel_s7_split_adj : ∀ x y : Fin 7 ⊕ Fin 0,
    (cycleGraph 7 7).Adj (kernelS7Split x) (kernelS7Split y) ↔
      match x, y with
...
theorem kernel_block_adj (a : Fin 7 ≃ Fin 7) (b : Fin 0 ≃ Fin 0)
    (ha : ∀ x y, (cycleGraph 7 7).Adj (a x) (a y) ↔ (cycleGraph 7 7).Adj x y) :
    ∀ x y, (cycleGraph 7 7).Adj (kernelBlockEquiv a b x) (kernelBlockEquiv a b y) ↔
...
theorem kernel_block_map_join (a : Fin 7 ≃ Fin 7) (b : Fin 0 ≃ Fin 0)
    (J : Finset (Fin 7)) (K : Finset (Fin 0)) :
...
def kernelIsoPrefix (k : Fin 1) : Finset (Fin 0) :=
  Finset.univ.filter (fun x => x.val < k.val)

theorem kernel_iso_prefix_card : ∀ k : Fin 1, (kernelIsoPrefix k).card = k.val := by
  decide +kernel

def kernelS7Pack (c : Fin 128) (k : Fin 1) : Fin 128 :=
  ⟨(c.val + 128 * (2 ^ k.val - 1)) % 128, Nat.mod_lt _ (by decide)⟩

-- Eight cycle shapes × isolate counts; the zero case is
-- retained explicitly, then excluded by nonemptiness in the general proof.
theorem kernel_s7_packed_shapes : ∀ c : Fin 128,
    c.val ∈ kernelC7CanonicalMasks → ∀ k : Fin 1,
